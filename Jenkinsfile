pipeline {
  ...
  stages {
    ...
    stage('Update Deployment File') {
        environment {
           GIT_REPO_NAME = "spring-app-july24"
        ...
        steps {
            withCredentials([string(credentialsId: 'github', variable: 'GITHUB_TOKEN')]) {
                sh '''
                    ...
                    sed -i "s+"newTag.*"+"newTag:PLUS_${BUILD_TAG}_"+g" kustomization.yaml
                    sed -i 's/_/"/g' kustomization.yaml
                    sed -i 's/PLUS/ /g' kustomization.yaml
                    git add kustomization.yaml
                    git commit -m "Update deployment image to version ${BUILD_NUMBER}"
                    git remote set-url origin https://sujayt-ghub:${GITHUB_TOKEN}@github.com/sujayt-ghub/spring-app-july24.git
